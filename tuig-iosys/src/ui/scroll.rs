...
use crate::{XY, Action, Key, MouseButton, fmt::Cell};

use super::{Region, Bounds};
...
impl<'s, 'st> ScrolledRegion<'s, 'st> {
    ...
    pub(crate) fn new(region: Region<'s>, state: &'st mut ScrollState) -> Self {
        Self { region, state, virt_size: XY(usize::MAX, usize::MAX), _dummy: Cell::BLANK }
    }
    ...
    pub fn bounds(&self) -> Bounds {
        Bounds { pos: self.state.pos, size: self.region.size() }
    }
    ...
    pub fn scroll_with_wheel(&mut self, input: &Action) -> bool {
        match input {
            Action::MousePress { button: MouseButton::ScrollUp, .. } => self.scroll_up(3),
            Action::MousePress { button: MouseButton::ScrollDown, .. } => self.scroll_down(3),
            _ => return false,
        }
        true
    }
